# Takes a sequence of bacteria positions over time and creates an unmerged table of d2 distances by iterating through gaps of time (deltaT) and each pair of positions in tracks.
# Able to summarize the table with an average d2 for each deltaT.

library(tidyverse)
# Import tracks as CSV. Format: track,slice,x,y.
raw <- read.csv("")

# Micron/pixel ratio. Adjust if needed.
scale = 0.2397

# Create empty 'pool' df containing all deltaT and d2.
...
for (numTrack in 1:length(unique(raw$track))){ #iterate track
  ...
  for (dt in 0:(nrow(t)-1)){ #iterate deltaT 
    for (i in 1:(nrow(t)-dt)){ #iterate through each pair of points
      subpool[nrow(subpool) + 1,] <- c(numTrack, dt, (((t$x[i+dt]-t$x[i])*scale)^2+((t$y[i+dt]-t$y[i])*scale)^2))
